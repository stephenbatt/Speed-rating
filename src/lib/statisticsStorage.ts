...
// Extract numeric Beyers from pastPerformances (newest-first order expected)
...
// Determine pattern from the most recent Beyers (newest-first)
const detectPattern = (speeds) => {
  ...
  if (hitMissHit) return 'hitMissHit';
  if (backingUp) return 'backingUp';
  return 'none';
};

const getHitSpeeds = (speeds) => {
  const hits = [];
  for (let i = 0; i < speeds.length; i++) {
    ...
    if (i === 0 && typeof next !== 'undefined') {
      if (current > next) hits.push(current);
      continue;
    }
...
const applyPlusFive = (speeds, pattern) => {
  const result = [...speeds];
  if (result.length === 0) return result;

  if (pattern === 'hitMissHit') {
    const hits = getHitSpeeds(result);
    const bestHit = hits.length > 0 ? Math.max(...hits) : Math.max(...result.slice(0, 2));
    result.push(bestHit + 5);
    return result;
  }

  const lastTwo = result.slice(0, 2);
  const bestLast2 = Math.max(...lastTwo);
  result.push(bestLast2 + 5);
  return result;
};

// Apply Stephen's one-time replacement rule
const applyReplacement = (speeds, pattern) => {
  if (!speeds || speeds.length === 0) return speeds;
  const result = [...speeds];

  if (pattern === 'improving') {
    if (result.length < 5) return result;

    const latestFour = result.slice(0, 4);
    const weakest = Math.min(...latestFour);
    const latestCandidates = [...latestFour];
    const weakIndex = latestCandidates.indexOf(weakest);
    if (weakIndex !== -1) {
      latestCandidates.splice(weakIndex, 1);
    }

    const projection = result[result.length - 1];
    return [...latestCandidates, projection];
  }

  if (pattern === 'hitMissHit') {
    const projection = result[result.length - 1];
    const hits = getHitSpeeds(result.slice(0, -1));
    if (hits.length === 0) return result;
    const hitPool = [projection, ...hits];
    return hitPool.sort((a, b) => b - a).slice(0, 3);
  }
...
export const calculateStephenTop3 = (pastPerformances) => {
  ...
  speeds = applyPlusFive(speeds, pattern);
