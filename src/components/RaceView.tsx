...
const ppPicks = useMemo(() => {
  return horses
    .filter(horse => horse.postPosition > 0)
    .sort((a, b) => parseOdds(a.odds) - parseOdds(b.odds))
    .slice(0, 4)
    .map(horse => horse.postPosition);
}, [horses]);

const ladderRankings = useMemo(() => {
  if (rankings.length === 0) return [];

  const topScore = rankings[0].adjustedScore;

  return rankings.map((r, index) => {
    const diff = topScore - r.adjustedScore;
    const adjustment = index === 0 ? 0 : -5 * Math.ceil(diff / 5);
...
const redEyePicks = useMemo(() => {
  return ladderRankings
    .slice(1, 5)
    .map(r => r.postPosition);
}, [ladderRankings]);

  // Used by compact view trophy icon.
  const topPicks = enginePicks;
              ...
              <div className="grid grid-cols-4 gap-6 text-center text-sm">
                <div>
                  ...
                  {ppPicks.length > 0 ? (
                    ...
                    <div className="text-sm font-bold text-blue-400">TBD</div>
                  )}
                </div>
                ...
                <div>
                  <div className="font-bold text-red-300">🔴 RED-EYE</div>
                  {redEyePicks.map((p, i) => (
                    <div key={i} className="text-xl font-bold text-red-400">
                      {p}
                    </div>
                  ))}
                </div>
            ...
            {sortedHorses.map(horse => {
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-1">
                    <span
                      className={`inline-flex w-8 h-8 items-center justify-center rounded-lg font-bold text-sm ${
                        horse.color === 'Red'
      ...
      {horses.length === 0 && (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500">
            Paste race data to see horse entries and past performances.
          </p>
        </div>
      )}
    </div>
