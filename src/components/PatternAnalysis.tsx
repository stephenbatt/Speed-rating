...
import {
  ...
  calculateRankings,
  analyzePatterns,
  computeStephenImprovingScore
} from '@/utils/raceDataParser';
...
const PatternAnalysis: React.FC<PatternAnalysisProps> = ({ horses, trackName, raceNumber }) => {
  ...
  const computeBeyerProfile = (horse: HorseData) => {
    ...
    const bestLastTwo = pa?.bestLastTwo ?? 0;
    const todayRating = bestLastTwo > 0 ? bestLastTwo + 5 : 0;
    const topThree = pa?.topThreeBeyer ?? [];
    const topThreeSum = pa?.topThreeBeyerSum ?? topThree.reduce((s, n) => s + n, 0);
