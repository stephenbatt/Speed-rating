...
const todaySpeed = bestLastTwo > 0 ? bestLastTwo + 5 : 0;
 ...
 // 🔥 USE UP TO 7 OUTS
const analysisSpeeds = validSpeeds.slice(0, 7);

// 🔥 DEFAULT IMPROVING HORSE
let topThreeBeyer = [
  todaySpeed,
  analysisSpeeds[1] || 0,
  analysisSpeeds[2] || 0,
...
// 🔥 USE ALL VALID SPEEDS
const speeds = [...validSpeeds].reverse(); // oldest → newest
...
export const computeStephenImprovingScore = (horse: HorseData): number => {
...
// --------------------------------------------------------------------------
// HIT / MISS HORSE
// use projection hit + next HIT in sequence
// --------------------------------------------------------------------------
...
// ============================================================================
  ...
  const rankings: HorseRanking[] = horses.map(h => {
    ...
    // Trust is informational only. Do not apply a penalty to the final score.
    const adjustment = 0;
  ...
  rankings.sort((a, b) => b.finalScore - a.finalScore || b.adjustedScore - a.adjustedScore);
  return rankings;export const calculateRankings = (horses: HorseData[]): HorseRanking[] => {
};